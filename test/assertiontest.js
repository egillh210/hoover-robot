...
fs.readFile(filename1, 'utf8', function(err, data) {
  ...
  data.pop();
  data[0] == input1[0] && data[1] == input1[1] ? console.log(assertion.yellow + '✅') : console.log(assertion.yellow + '❌');
})
...
fs.readFile(filename2, 'utf8', function(err, data) {
  ...
  data = data.split('\n').filter(str => str.length > 0);
  data.pop();
  data[0] == input2[0] && data[1] == input2[1] ? console.log(assertion.yellow + '✅') : console.log(assertion.yellow + '❌');
})
...
fs.readFile(filename3, 'utf8', function(err, data) {
  ...
  data = data.split('\n').filter(str => str.length > 0);
  data.pop();
  data[0] == input3[0] && data[1] == input3[1] ? console.log(assertion.yellow + '✅') : console.log(assertion.yellow + '❌');
})
